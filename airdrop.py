...
def initialize():
    ...
    Config.create(address="0x0000000000000000000000000000000000000000",
                  ...
                  web3_node="https://data-seed-prebsc-1-s1.binance.org:8545/",
                  token="0x0000000000000000000000000000000000000000",
                  caribmars_token="0x0000000000000000000000000000000000000000")
...
def set_caribmars_token(token_address):
#    if not Web3.isChecksumAddress(token_address):
#        print('Wrong token address. Try again.')
#        return

    config = Config.get(1)
    config.caribmars_token = token_address
    config.save()
    print(f"Now caribmars token for airdrop is {token_address}")

def get_holder():
    config = Config.get(1)
    w3 = Web3(Web3.HTTPProvider(config.web3_node, request_kwargs={"timeout": 20}))
    with open("tokens_abi/CaribMars.abi", "r") as file:
        bep20_abi = json.load(file)
...
def update_data():
    ...
    token_contract = w3.eth.contract(address=Web3.toChecksumAddress(config.token), abi=bep20_abi)
    try:
...
def sign():
    ...
    with open("tokens_abi/ERC20.abi", "r") as file:
        bep20_abi = json.load(file)
    token = w3.eth.contract(address=Web3.toChecksumAddress(config.token), abi=bep20_abi)
    nonce = config.current_nonce
    ...
    for recipient in recipients:
        ...
        try:
            raw_tx = token.functions.transfer(
                                recipient.address,
                                int(float(recipient.amount) * 10**9),
                                ).buildTransaction({
                                                    "from": config.address,
                                                    "nonce": nonce,
            ...
            print('Not enough BNB or Token balance. Fill your balance and try again.')
            return
        tx.raw_tx = raw_tx
...
def send():
    ...
    try:
        ...
        if str(e) == "{'code': -32000, 'message': 'insufficient funds for gas * price + value'}":
            print('Not enough BNB Balance. Fill your balance and try again.')
...
def help():
    ...
    caribmars_token <address>   Set caribmars token address for airdrop.
...
command_dict = {
    "init": initialize,
    'import': import_key,
    'caribmars_token': set_caribmars_token,
